...
static expression_t __parser_expression__(parser_t parse)
{
    ...
    default:
        if (lvalue_exprs_total >= 1) {
            expr = expression_new_list(line, column, lvalue_exprs);
        } else {
    ...
    if (lvalue_exprs_total > 1 || lvalue_exprs_total == 0) {
        error(tok->filename, line, column, "unexpected lvalue expression");
    }
...
static expression_t __parser_lvalue_expression__(parser_t parse)
{
    ...
    case TOKEN_VALUE_IDENTIFIER:
        lvalue_expr = expression_new_identifier(line, column, cstring_dup(tok->token));
        lexer_next(parse->lex);
        break;
...
static expression_t __parser_logical_or_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_OR) {
        lexer_next(parse->lex);
        expr    = expression_new_binary(EXPRESSION_TYPE_OR, line, column, expr, __parser_logical_and_expression__(parse));
        tok     = lexer_peek(parse->lex);
        line    = tok->line;
        column  = tok->column;
    }
...
static expression_t __parser_logical_and_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_AND) {
        lexer_next(parse->lex);
        expr   = expression_new_binary(EXPRESSION_TYPE_AND, line, column, expr, __parser_equality_expression__(parse));
        tok    = lexer_peek(parse->lex);
        line   = tok->line;
        column = tok->column;
    }
...
static expression_t __parser_equality_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_EQ || 
        ...
        switch (tv) {
        case TOKEN_VALUE_EQ:
            expr = expression_new_binary(EXPRESSION_TYPE_EQ, line, column, expr, __parser_relational_expression__(parse));
            break;

        case TOKEN_VALUE_NEQ:
            expr = expression_new_binary(EXPRESSION_TYPE_NEQ, line, column, expr, __parser_relational_expression__(parse));
            break;
...
static expression_t __parser_relational_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_GT || 
        ...
        switch (tv) {
        case TOKEN_VALUE_GT:
            expr = expression_new_binary(EXPRESSION_TYPE_GT, line, column, expr, __parser_additive_expression__(parse));
            break;

        case TOKEN_VALUE_GEQ:
            expr = expression_new_binary(EXPRESSION_TYPE_GEQ, line, column, expr, __parser_additive_expression__(parse));
            break;

        case TOKEN_VALUE_LT:
            expr = expression_new_binary(EXPRESSION_TYPE_LT, line, column, expr, __parser_additive_expression__(parse));
            break;

        case TOKEN_VALUE_LEQ:
            expr = expression_new_binary(EXPRESSION_TYPE_LEQ, line, column, expr, __parser_additive_expression__(parse));
            break;
...
static expression_t __parser_additive_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_ADD || 
        ...
        case TOKEN_VALUE_ADD:
            expr = expression_new_binary(EXPRESSION_TYPE_ADD, line, column, expr, __parser_multiplicative_expression__(parse));
            break;

        case TOKEN_VALUE_SUB:
            expr = expression_new_binary(EXPRESSION_TYPE_SUB, line, column, expr, __parser_multiplicative_expression__(parse));
            break;
...
static expression_t __parser_multiplicative_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_MUL ||
        ...
        case TOKEN_VALUE_MUL:
            expr = expression_new_binary(EXPRESSION_TYPE_MUL, line, column, expr, __parser_bitop_expression__(parse));
            break;

        case TOKEN_VALUE_DIV:
            expr = expression_new_binary(EXPRESSION_TYPE_DIV, line, column, expr, __parser_bitop_expression__(parse));
            break;

        case TOKEN_VALUE_MOD:
            expr = expression_new_binary(EXPRESSION_TYPE_MOD, line, column, expr, __parser_bitop_expression__(parse));
            break;
...
static expression_t __parser_bitop_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_BITAND ||
        ...
        case TOKEN_VALUE_BITAND:
            expr = expression_new_binary(EXPRESSION_TYPE_BITAND, line, column, expr, __parser_shift_bitop_expression__(parse));
            break;

        case TOKEN_VALUE_BITOR:
            expr = expression_new_binary(EXPRESSION_TYPE_BITOR, line, column, expr, __parser_shift_bitop_expression__(parse));
            break;

        case TOKEN_VALUE_XOR:
            expr = expression_new_binary(EXPRESSION_TYPE_XOR, line, column, expr, __parser_shift_bitop_expression__(parse));
            break;
...
static expression_t __parser_shift_bitop_expression__(parser_t parse)
{
    ...
    while (tok->value == TOKEN_VALUE_LEFT_SHIFT ||
        ...
        case TOKEN_VALUE_LEFT_SHIFT:
            expr = expression_new_binary(EXPRESSION_TYPE_LEFT_SHIFT, line, column, expr, __parser_unary_expression__(parse));
            break;

        case TOKEN_VALUE_RIGHT_SHIFT:
            expr = expression_new_binary(EXPRESSION_TYPE_RIGHT_SHIFT, line, column, expr, __parser_unary_expression__(parse));
            break;

        case TOKEN_VALUE_LOGIC_RIGHT_SHIFT_ASSIGN:
            expr = expression_new_binary(EXPRESSION_TYPE_LOGIC_RIGHT_SHIFT, line, column, expr, __parser_unary_expression__(parse));
            break;
...
static expression_t __parser_array_generate_expression__(parser_t parse)
{
    ...
    while (tok->type != TOKEN_TYPE_END && tok->value != TOKEN_VALUE_RB) {
        assert((elem = __parser_expression__(parse)) != NULL);
...
static expression_t __parser_table_generate_expression__(parser_t parse)
{
    ...
    while (tok->type != TOKEN_TYPE_END && tok->value != TOKEN_VALUE_RC) {
        ...
        pair = expression_new_table_pair(elemname, elem);
